...
export default function PajeroDetail() {
  const [seriesIdx, setSeriesIdx] = useState(0);
  const [variantIdx, setVariantIdx] = useState(0);
  const [colorIdx, setColorIdx] = useState(0);

  const activeSeries = pajeroSeries[seriesIdx];
  ...
  const hasMultipleVariants = activeSeries.variants.length > 1;
  ...
  const brochureFile = "/brochures/pajero-sport-brochure.pdf";
  ...
  const testDriveLink = getWaLink(
    `Halo ${siteConfig.sales.name}, saya ingin *test drive Pajero Sport*. Mohon dijadwalkan. Terima kasih.`,
  );
    ...
    <>
      ...
      <section className="relative min-h-[600px] h-[80svh] w-full overflow-hidden bg-mitsu-black">
        <video
          ...
          playsInline
          preload="auto"
          poster="/cars/ps-dult4x4-gray.webp"
        >
          <source src="/video/ps-hero-banner.mp4" type="video/mp4" />
        </video>
          ...
          <div className="mx-auto w-full max-w-7xl px-5 lg:px-8">
            ...
            >
              ...
              <h1 className="mt-5 text-4xl font-black leading-[0.95] text-white sm:text-6xl lg:text-7xl drop-shadow-lg">
                Dominasi Setiap<br />Perjalanan
              </h1>
              <p className="mt-4 sm:mt-6 max-w-md text-sm sm:text-base leading-relaxed text-white/80 drop-shadow">
                SUV tangguh 7-seater bermesin 2.4L Turbo Diesel dengan Super Select
                4WD-II, siap menaklukkan segala medan.
              </p>
              ...
              <p className="text-3xl sm:text-4xl font-black text-white drop-shadow-lg">
                {formatRupiah(pajeroStartingPrice)}
              </p>
        ...
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          ...
          <div className="mt-12 grid gap-8 overflow-hidden rounded-3xl bg-white shadow-luxe lg:grid-cols-[1.4fr_1fr]">
            ...
            <div className="flex flex-col">
              ...
              {/* Drivetrain / transmission toggle */}
              {hasMultipleVariants && (
            ...
            <div className="flex flex-col justify-center border-t border-black/5 p-6 sm:p-8 lg:border-l lg:border-t-0">
              <h3 className="text-2xl font-black text-mitsu-black">Pajero Sport</h3>
              <p className="mt-1 text-lg font-semibold text-mitsu-gray">
                {activeSeries.label} {activeVariant.label}
              </p>
      ...
      <section id="spesifikasi-ps" className="scroll-mt-20 bg-white py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          <div className="text-center">
            ...
            <table className="w-full min-w-[860px] border-collapse text-left text-xs sm:text-sm">
              <thead>
                <tr className="bg-mitsu-red text-white">
                  <th className="sticky left-0 z-10 bg-mitsu-red px-3 py-3 font-bold uppercase tracking-wide">Spesifikasi</th>
                  {pajeroSpecColumns.map((c) => (
                    <th key={c} className="px-3 py-3 text-center font-bold uppercase tracking-wide whitespace-nowrap">{c}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {pajeroSpecTable.map((group) => (
                  <Fragment key={group.title}>
                    <tr className="bg-mitsu-black">
                      <td colSpan={7} className="sticky left-0 z-10 bg-mitsu-black px-3 py-2.5 text-xs font-bold uppercase tracking-wider text-white">
                        {group.title}
                      </td>
                    </tr>
                    {group.rows.map((row, ri) => {
                      const rowBg = ri % 2 === 0 ? "#ffffff" : "#eef0f2";
                      return (
                        <tr key={`${group.title}-${row.label}`} style={{ backgroundColor: rowBg }}>
                          <td
                            className="sticky left-0 z-10 px-3 py-2.5 align-top text-mitsu-black shadow-[2px_0_4px_-2px_rgba(0,0,0,0.15)]"
      ...
      <section className="bg-mitsu-black py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          <div className="text-center">
            ...
            <h2 className="mt-3 text-3xl font-black text-white sm:text-4xl">Tangguh Tanpa Kompromi</h2>
          </div>
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {pajeroFeatures.map((f) => (
      ...
      <section className="bg-white py-16 sm:py-24">
        <div className="mx-auto max-w-5xl px-5 lg:px-8">
          <div className="text-center">
            ...
            <h2 className="mt-3 text-3xl font-black text-mitsu-black sm:text-4xl">Kenali Lebih Dekat Pajero Sport</h2>
          </div>
          <div className="mt-12 overflow-hidden rounded-2xl bg-black shadow-luxe">
            <div className="relative aspect-video w-full">
              <iframe
                src={`https://www.youtube-nocookie.com/embed/${pajeroYoutubeId}`}
                title="Pajero Sport — Mitsubishi Dipo Serang"
      ...
      <section className="bg-gradient-to-r from-mitsu-black to-mitsu-red/80 py-16 sm:py-20">
        <div className="mx-auto max-w-4xl px-5 text-center lg:px-8">
          <h2 className="text-3xl font-black text-white sm:text-4xl">Siap Memiliki Pajero Sport?</h2>
